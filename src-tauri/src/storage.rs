...
impl Storage {
    pub fn initialize(app: &AppHandle) -> Result<Self, String> {
        let app_dir = app.path().app_data_dir().map_err(|error| format!("无法确定应用数据目录：{error}"))?;
        ...
        let connection = Connection::open(&database_path).map_err(|error| format!("无法打开本地数据库：{error}"))?;
        ...
        let storage = Self { connection: Mutex::new(connection), database_path };
        storage.housekeeping()?;
        Ok(storage)
    }

    pub fn provider_config(&self) -> Result<ProviderConfig, String> {
        let connection = self.connection.lock().map_err(|_| "数据库锁已损坏".to_string())?;
        let stored: Option<String> = connection.query_row(
            "SELECT value_json FROM app_settings WHERE key = 'provider.deepseek'",
            [],
            |row| row.get(0),
        ).ok();
        let mut config: ProviderConfig = stored.and_then(|json| serde_json::from_str(&json).ok()).unwrap_or_default();
    ...
    pub fn save_provider_config(&self, config: &ProviderConfig) -> Result<(), String> {
        if let Some(api_key) = config.api_key.as_ref().filter(|key| !key.trim().is_empty()) {
            Entry::new(SERVICE, API_KEY_ACCOUNT).map_err(|error| format!("无法访问 Windows 凭据管理器：{error}"))?
                .set_password(api_key.trim()).map_err(|error| format!("无法保存 API Key：{error}"))?;
        }
    ...
    pub fn api_key(&self) -> Result<String, String> {
        Entry::new(SERVICE, API_KEY_ACCOUNT).map_err(|error| format!("无法访问 Windows 凭据管理器：{error}"))?
            .get_password().map_err(|_| "尚未配置 DeepSeek API Key".to_string())
    }
    ...
    pub fn list_history(&self, query: Option<&str>) -> Result<Vec<HistoryRecord>, String> {
        ...
        let rows = statement.query_map([pattern], |row| Ok(HistoryRecord {
            id: row.get(0)?, title: row.get(1)?, original: row.get(2)?, enhanced: row.get(3)?,
            created_at: row.get(4)?, model: row.get(5)?, target: row.get(6)?,
        })).map_err(|error| error.to_string())?;
    ...
    pub fn delete_history(&self, id: &str) -> Result<(), String> {
        let connection = self.connection.lock().map_err(|_| "数据库锁已损坏".to_string())?;
        connection.execute("DELETE FROM history WHERE id=?1", [id]).map_err(|error| error.to_string())?;
        Ok(())
    }
    ...
    pub fn app_settings(&self) -> Result<AppSettings, String> {
        let connection = self.connection.lock().map_err(|_| "数据库锁已损坏".to_string())?;
        let stored: Option<String> = connection.query_row(
            "SELECT value_json FROM app_settings WHERE key = 'ui.local'", [], |row| row.get(0),
        ).ok();
    ...
    pub fn clear_all_data(&self) -> Result<(), String> {
        let mut connection = self.connection.lock().map_err(|_| "数据库锁已损坏".to_string())?;
        let transaction = connection.transaction().map_err(|error| error.to_string())?;
        transaction.execute("DELETE FROM history", []).map_err(|error| error.to_string())?;
        transaction.execute("DELETE FROM usage_records", []).map_err(|error| error.to_string())?;
        transaction.execute("DELETE FROM app_settings", []).map_err(|error| error.to_string())?;
        transaction.commit().map_err(|error| error.to_string())?;
        if let Ok(entry) = Entry::new(SERVICE, API_KEY_ACCOUNT) { let _ = entry.delete_credential(); }
        Ok(())
    }
    ...
    pub fn export_data(&self, path: &Path) -> Result<usize, String> {
        ...
        for (name, value) in [
            ("manifest.json", serde_json::to_vec_pretty(&manifest).map_err(|error| error.to_string())?),
            ("history.json", serde_json::to_vec_pretty(&history).map_err(|error| error.to_string())?),
            ("settings.json", serde_json::to_vec_pretty(&settings).map_err(|error| error.to_string())?),
            ("provider.json", serde_json::to_vec_pretty(&provider).map_err(|error| error.to_string())?),
        ] {
            writer.start_file(name, options).map_err(|error| format!("无法写入导出包：{error}"))?;
            writer.write_all(&value).map_err(|error| format!("无法写入导出包：{error}"))?;
        }
        writer.finish().map_err(|error| format!("无法完成导出：{error}"))?;
        Ok(history.len())
    }

    pub fn import_data(&self, path: &Path) -> Result<usize, String> {
        let metadata = fs::metadata(path).map_err(|error| format!("无法读取导入文件：{error}"))?;
        if metadata.len() > MAX_ARCHIVE_BYTES { return Err("导入包不能超过 100 MB".into()); }
        let file = fs::File::open(path).map_err(|error| format!("无法打开导入文件：{error}"))?;
        let mut archive = ZipArchive::new(file).map_err(|_| "导入文件不是有效的 ZIP 包".to_string())?;
        if archive.len() > 8 { return Err("导入包包含过多文件".into()); }
        let allowed = ["manifest.json", "history.json", "settings.json", "provider.json"];
        let mut total_uncompressed = 0_u64;
        for index in 0..archive.len() {
            let entry = archive.by_index(index).map_err(|error| error.to_string())?;
            let enclosed = entry.enclosed_name().and_then(|path| path.to_str().map(str::to_owned)).ok_or_else(|| "导入包包含不安全路径".to_string())?;
            if !allowed.contains(&enclosed.as_str()) { return Err(format!("导入包包含不支持的内容：{enclosed}")); }
            total_uncompressed = total_uncompressed.saturating_add(entry.size());
        }
        if total_uncompressed > MAX_ARCHIVE_BYTES { return Err("导入包解压后的内容超过 100 MB".into()); }
    ...
    pub fn record_usage(&self, model: &str, input_tokens: u64, output_tokens: u64, cost: f64, duration_ms: u64, status: &str, error_code: Option<&str>) -> Result<f64, String> {
        let connection = self.connection.lock().map_err(|_| "数据库锁已损坏".to_string())?;
        connection.execute(
    ...
    pub fn housekeeping(&self) -> Result<(), String> {
        ...
        let connection = self.connection.lock().map_err(|_| "数据库锁已损坏".to_string())?;
        connection.execute("DELETE FROM history WHERE pinned=0 AND created_at < ?1", [&cutoff]).map_err(|error| error.to_string())?;
        connection.execute("DELETE FROM usage_records WHERE created_at < ?1", [&cutoff]).map_err(|error| error.to_string())?;
        if fs::metadata(&self.database_path).map(|metadata| metadata.len()).unwrap_or(0) > 500 * 1024 * 1024 {
            connection.execute(
...
fn read_archive_json<T: for<'de> Deserialize<'de>>(archive: &mut ZipArchive<fs::File>, name: &str) -> Result<T, String> {
    let mut entry = archive.by_name(name).map_err(|_| format!("导入包缺少 {name}"))?;
    let mut bytes = Vec::with_capacity(entry.size().min(4 * 1024 * 1024) as usize);
    entry.read_to_end(&mut bytes).map_err(|error| format!("无法读取 {name}：{error}"))?;
    serde_json::from_slice(&bytes).map_err(|_| format!("{name} 内容格式无效"))
}

fn get_or_create_database_key() -> Result<String, String> {
    let entry = Entry::new(SERVICE, DB_KEY_ACCOUNT).map_err(|error| format!("无法访问 Windows 凭据管理器：{error}"))?;
    if let Ok(existing) = entry.get_password() { return Ok(existing); }
    let bytes: [u8; 32] = rand::random();
    let key = hex::encode(bytes);
    entry.set_password(&key).map_err(|error| format!("无法保存数据库密钥：{error}"))?;
    Ok(key)
}
