...
JUDGE_SYSTEM_PROMPT = """你是公正的模型评测裁判。给定一个任务提示词及其回答，请对回答从四个维度各打 1-10 分（整数）：
...
{"accuracy":1,"completeness":1,"relevance":1,"clarity":1,"reason":"一句话理由"}"""
...
def judge_answer(
    ...
    """对单个回答四维度打分，返回 {accuracy, completeness, relevance, clarity, reason}。
...
def _parse_scores(text: str) -> dict[str, Any]:
    """解析裁判 JSON：四维度必须是 1-10 的整数，reason 缺失时置空。"""
